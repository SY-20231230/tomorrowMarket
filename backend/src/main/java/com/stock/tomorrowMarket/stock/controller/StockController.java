...
public class StockController {
    ...
    @Operation(summary = "종목코드로 주식 상세 조회", description = "종목코드를 통해 주식의 기본 정보와 최근 종가를 조회합니다. 로그인 시 검색 기록이 자동 저장됩니다.")
    @GetMapping("/code/{stockCode}")
    public ApiResponse<StockDetailResponse> getStockDetailByCode(
            @PathVariable String stockCode,
            @AuthenticationPrincipal CustomUserDetails userDetails
    ) {
        Long userId = (userDetails != null) ? userDetails.getUsersId() : null;
        StockDetailResponse response = stockService.getStockDetailByCode(stockCode, userId);
        return ApiResponse.success(response);
    }
